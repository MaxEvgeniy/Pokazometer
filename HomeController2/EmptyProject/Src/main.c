...
uint32_t ReadSensorsTaskBuffer[ 128 ];
osStaticThreadDef_t ReadSensorsTaskControlBlock;
const osThreadAttr_t ReadSensorsTask_attributes = {
  .name = "ReadSensorsTask",
  .cb_mem = &ReadSensorsTaskControlBlock,
  .cb_size = sizeof(ReadSensorsTaskControlBlock),
  .stack_mem = &ReadSensorsTaskBuffer[0],
  .stack_size = sizeof(ReadSensorsTaskBuffer),
  .priority = (osPriority_t) osPriorityLow,
};
/* Definitions for MQTTTask */
osThreadId_t MQTTTaskHandle;
uint32_t MQTTTaskBuffer[ 128 ];
...
uint8_t MQTTMessQueueBuffer[ 16 * sizeof( uint16_t ) ];
...
int main(void)
{
  ...
  MQTTMessQueueHandle = osMessageQueueNew (16, sizeof(uint16_t), &MQTTMessQueue_attributes);
