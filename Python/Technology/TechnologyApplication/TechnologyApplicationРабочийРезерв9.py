...
class App:
    def __init__(self, root, pdf_path, обозначение, наименование, материал, разработал, дата):
        ...
        # Поле: Обозначение
        tk.Label(frame_right, text="Обозначение:", font=("Arial", 14)).pack(anchor=tk.W, padx=20, pady=(10, 0))
        self.oboznachenie_var = tk.StringVar(value=self.обозначение or "")
        self.oboznachenie_entry = tk.Entry(frame_right, textvariable=self.oboznachenie_var, font=("Arial", 12), width=40)
        self.oboznachenie_entry.pack(padx=20, pady=5)

        # Поле: Наименование
        tk.Label(frame_right, text="Наименование:", font=("Arial", 14)).pack(anchor=tk.W, padx=20, pady=(10, 0))
        self.naimenovanie_var = tk.StringVar(value=self.наименование or "")
        self.naimenovanie_entry = tk.Entry(frame_right, textvariable=self.naimenovanie_var, font=("Arial", 12), width=40)
        self.naimenovanie_entry.pack(padx=20, pady=5)

        # Поле: Фамилия (Разработал)
        tk.Label(frame_right, text="Фамилия:", font=("Arial", 14)).pack(anchor=tk.W, padx=20, pady=(10, 0))
        self.surname_var = tk.StringVar(value=self.разработал or "")
        self.surname_entry = tk.Entry(frame_right, textvariable=self.surname_var, font=("Arial", 12), width=40)
        self.surname_entry.pack(padx=20, pady=5)

        # Поле: Дата
        tk.Label(frame_right, text="Дата:", font=("Arial", 14)).pack(anchor=tk.W, padx=20, pady=(10, 0))
        self.date_var = tk.StringVar(value=self.дата or "")
        self.date_entry = tk.Entry(frame_right, textvariable=self.date_var, font=("Arial", 12), width=40)
        ...
        self.material_values = load_materials(excel_file, sheet_name="Лист")
        ...
        if os.path.exists(excel_file):
            self.types = load_types(excel_file)
        else:
    ...
    def update_checkboxes(self):
        ...
        # Обновляем материалы
        self.material_values = load_materials(excel_file, sheet_name=selected_type)
        ...
        # Загружаем операции
        operations = load_operations(excel_file, selected_type)
    ...
    def on_continue(self):
        ...
        self.разработал = self.surname_var.get().strip()
        self.обозначение = self.oboznachenie_var.get().strip()
        self.наименование = self.naimenovanie_var.get().strip()
        self.дата = self.date_var.get().strip()
...
def replace_all(document, old_text, new_text):
    """
    Точная замена ключевых слов с сохранением стиля первого run
    """
    replaced = False
    old_cleaned = re.sub(r'\s+', '', old_text).lower()

    for para in document.paragraphs:
        full_text = ''.join(run.text for run in para.runs)
        if re.sub(r'\s+', '', full_text).lower() == old_cleaned:
            if para.runs:
...
def extract_and_save_image(pdf_path, output_path, area=None, zoom_preview=0.7, zoom_export=6):
    try:
        doc = fitz.open(pdf_path)
        page = doc.load_page(0)
...
def load_materials(excel_file, sheet_name):
    try:
...
def load_operations(excel_file, sheet_name):
    try:
        df = pd.read_excel(excel_file, sheet_name=sheet_name)
        operations = {}
        for _, row in df.iterrows():
...
def save_as_docx(pdf_path, template_path, output_path, replacements):
    try:
        document = Document(template_path)

        # Замена всех ключевых слов
        for key, value in replacements.items():
            replace_all(document, key, value)
        ...
        extract_and_save_image(pdf_path, temp_image_path, area_to_use, zoom_preview=app.zoom_level, zoom_export=6)
        success = insert_image(document, "#Картинка", temp_image_path)

        if not success:
            print("[!] Не удалось вставить изображение")

        # Сохраняем как .docx
        document.save(output_path)
        print(f"[OK] Файл сохранён как {output_path}")
        return True
    except Exception as e:
        print(f"[!] Ошибка сохранения файла как .docx: {e}")
        return False
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith(".pdf"):
            ...
            try:
                ...
                # --- Подготовка замен ---
                replacements = {
                    "#ОбозначениеДетали": app.oboznachenie_var.get(),
                    "#НаименованиеДетали": app.naimenovanie_var.get(),
                    "#МатериалДетали": app.material_var.get(),
                    "#РазработалДеталь": app.surname_var.get(),
                    "#ДатаЧертежа": app.date_var.get(),
                    "#ТипЗаготовки": app.выбранный_тип or "",
                }

                if app.выбранный_тип != "Сборка":
                    replacements["#ОписаниеТехноОперации1"] = "Получение материала со склада в соответствии с сменным заданием и чертежом."
                    replacements["#СкладОперации1"] = "Склад заготовительного участка механического цеха."
                else:
                    replacements["#ОписаниеТехноОперации1"] = "Получение деталей со склада в соответствии со спецификацией."
                    replacements["#СкладОперации1"] = "Склад деталей сборочного цеха."

                replacements["#ОборудованиеОперации1"] = ""
                replacements["#ПриспособлениеОперации1"] = ""

                # Последующие операции
                for idx, op in enumerate(app.выбранные_операции, start=2):
                    data = get_operation_data(excel_file, app.выбранный_тип, op)
                    replacements[f"#ОписаниеТехноОперации{idx}"] = data["description"]
                    replacements[f"#ОборудованиеОперации{idx}"] = data["equipment"]
                    replacements[f"#ПриспособлениеОперации{idx}"] = data["tool"]
                    replacements[f"#СкладОперации{idx}"] = data["storage"]
                ...
                if os.path.exists(template_docx_path):
                    if save_as_docx(file_path, template_docx_path, output_path, replacements):
                        print(f"[OK] Сохранён файл: {output_path}")
                    else:
                        print(f"[!] Не удалось обработать: {output_path}")
                else:
                    print(f"[!] Шаблон '{template_docx_path}' не найден.")
            except Exception as e:
