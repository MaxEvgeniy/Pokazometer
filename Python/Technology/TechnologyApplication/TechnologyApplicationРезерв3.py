...
class App:
    ...
    def update_checkboxes(self):
        """Обновляет список чекбоксов при смене радиокнопки"""
        ...
        # Получаем текущий тип заготовки
    ...
    def toggle_continue(self):
        """Активирует кнопку Продолжить, если есть выбранные операции"""
    ...
    def on_continue(self):
        """Сохраняем результаты после закрытия окна"""
...
for filename in os.listdir(folder_path):
    if filename.lower().endswith(".pdf"):
        ...
        try:
            ...
            print(f"[Ошибка] Не удалось обработать файл '{filename}': {e}")
            try:
                doc_pdf.close()
            except:
                pass
