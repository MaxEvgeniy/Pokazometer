...
def load_types_from_excel(excel_file):
    """Читает типы заготовок из Excel"""
    try:
        ...
        return {"Лист": True, "Сборка": False}


def load_operations_with_columns(excel_file, sheet_name):
    """Загружает операции, оборудование, приспособления и склад с листа Excel"""
    try:
...
def get_operation_data(excel_file, sheet_name, operation_name):
    """Возвращает данные по операции: описание, оборудование, приспособление, склад"""
    try:
        df = pd.read_excel(excel_file, sheet_name=sheet_name)
        ...
        if not row.empty:
            ...
            print(f"[!] Не найдено данных для '{operation_name}' на листе '{sheet_name}'")
            ...
            try:
                ...
                try:
                    ...
                    if app.выбранный_тип != "Сборка":
                        ...
                        replace_all(document, "#СкладОперации1",
                                    "Склад заготовительного участка механического цеха.")
                    else:
                        replace_all(document, "#ОписаниеТехноОперации1",
                                    "Получение деталей со склада в соответствии со спецификацией.")
                        replace_all(document, "#СкладОперации1",
                                    "Склад деталей сборочного цеха.")
